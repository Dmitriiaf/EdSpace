...
import {
    ...
    MenuItem, Alert, IconButton, Grid, Stack, Divider, Autocomplete
} from '@mui/material';
import { styled } from '@mui/material/styles';
import {
    Cancel as CancelIcon, Event as EventIcon,
    CalendarMonth as BulkIcon,
...
function AdminSchedule({
    ...
    const handleCreateLesson = async () => {
        if (!createForm.studentIds || createForm.studentIds.length === 0) return;
        try {
            ...
            if (createForm.studentIds.length === 1) {
                await axiosInstance.post('/admin/lessons', {
                    studentId: createForm.studentIds[0],
                    tutorId: tutorId,
                ...
                await axiosInstance.post('/admin/lessons/pair', {
                    tutorId: tutorId,
                ...
                onShowSnackbar?.(`✅ Парное занятие создано для ${createForm.studentIds.length} учеников`, 'success');
            }
                                ...
                                <Box key={dayIdx} sx={{ flex: 1, position: 'relative', borderLeft: '1px solid #F3F4F6', height: timeSlots.length * HOUR_HEIGHT }}>
                                    ...
                                    {dayLessons.map(lesson => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <Typography sx={{ fontSize: '11px', fontWeight: 600, color: statusStyle.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                    {lesson.groupId ? '👥 ' : ''}{lesson.studentName}
                                                </Typography>
                ...
                <StyledDialog open={openCreateDialog} onClose={() => setOpenCreateDialog(false)} maxWidth="sm" fullWidth>
                    ...
                    <DialogContent>
                        <Stack spacing={2.5} sx={{ mt: 1 }}>
                            ...
                            <Autocomplete
                                multiple
                                options={tutorStudents}
                                getOptionLabel={(option) => option.fullName || ''}
                                value={tutorStudents.filter(s => createForm.studentIds.includes(s.id))}
                                onChange={(e, newValue) => setCreateForm({ ...createForm, studentIds: newValue.map(s => s.id) })}
                                renderInput={(params) => (
                                    <TextField
                                        {...params}
                                        label="Ученики"
                                        placeholder={createForm.studentIds.length === 0 ? 'Выберите одного или нескольких' : ''}
                                    />
                                )}
                            ...
                            {createForm.studentIds.length >= 2 && (
                                <Alert severity="info" sx={{ borderRadius: 2 }}>
                                    👥 Парное занятие для <b>{createForm.studentIds.length}</b> учеников.
                                    Общая видеокомната, у каждого ученика спишется по 1 занятию.
                                </Alert>
                            )}
                    ...
                    <DialogActions>
                        ...
                        <Button
                            ...
                            disabled={createForm.studentIds.length === 0}
                            sx={{ bgcolor: '#4F46E5' }}
                        >
                            Создать
                        </Button>
                    ...
                    <DialogContent>
                        <Stack spacing={2.5} sx={{ mt: 1 }}>
                            ...
                            <Autocomplete
                                ...
                                options={tutorStudents}
                                getOptionLabel={(option) => option.fullName || ''}
                                value={tutorStudents.filter(s => bulkForm.studentIds.includes(s.id))}
                                onChange={(e, newValue) => setBulkForm({ ...bulkForm, studentIds: newValue.map(s => s.id) })}
                                renderInput={(params) => (
                                    <TextField
                                        {...params}
                                        label="Ученики"
                                        placeholder={bulkForm.studentIds.length === 0 ? 'Выберите одного или нескольких' : ''}
                                    />
                                )}
                                renderTags={(value, getTagProps) =>
                                    value.map((option, index) => (
                            ...
                            {bulkForm.studentIds.length >= 2 && (
                                <Alert severity="info" sx={{ borderRadius: 2 }}>
                                    👥 Парные занятия для <b>{bulkForm.studentIds.length}</b> учеников.
                                    Общая комната, у каждого ученика списывается по 1 занятию.
                                </Alert>
                            )}
                            ...
                            <Alert severity="success" sx={{ borderRadius: 2 }}>
                                Будет создано <strong>{estimatedLessons * Math.max(bulkForm.studentIds.length, 1)}</strong> уроков
                            </Alert>
                        </Stack>
                    </DialogContent>
                    <DialogActions>
                        ...
                        <Button
                            ...
                            disabled={bulkLoading || bulkForm.studentIds.length === 0 || bulkForm.daysOfWeek.length === 0}
                            sx={{ bgcolor: '#10B981', '&:hover': { bgcolor: '#059669' } }}
                        >
                            {bulkLoading ? 'Создание...' : `Создать ${estimatedLessons * Math.max(bulkForm.studentIds.length, 1)} уроков`}
                        </Button>
                    </DialogActions>
                </StyledDialog>
